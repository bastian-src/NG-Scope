...
bool a_larger_than_b(int a, int b, int buf_size)
{
  if ((a - b) >= 0) {
    ...
    if ((abs(buf_size - b) < buf_size / 8) && (a < buf_size / 8)) {
      return true;
    }
  }
  return false;
...
void enqueue_dci_sf(sf_status_t* q, uint16_t targetRNTI, ngscope_status_buffer_t* dci_buffer)
{
  ...
  memcpy(&(q->dl_msg), &(dci_buffer->dci_per_sub.dl_msg), MAX_DCI_PER_SUB * sizeof(ngscope_dci_msg_t));
...
int push_rnti_dci_to_remote(sf_status_t* q, int cell_idx, int remote_sock)
{
	...
	cell_dci.timestamp_us = q->timestamp_us;
	cell_dci.tti        = q->tti;
	cell_dci.nof_rnti = 0;
	cell_dci.total_dl_tbs_bit = 0;
	cell_dci.total_dl_prb = 0;
	cell_dci.total_dl_no_tbs_prb = 0;

	cell_dci.total_ul_tbs_bit = 0;
	...
	for (int i = 0; i < nof_dl_msg; i++) {
		...
		rnti_dci->dl_tbs_bit += dci_msg.tb[0].tbs + dci_msg.tb[1].tbs;
		cell_dci.total_dl_tbs_bit += dci_msg.tb[0].tbs + dci_msg.tb[1].tbs;
	...
	for (int i = 0; i < nof_ul_msg; i++) {
		...
		rnti_dci->ul_tbs_bit += dci_msg.tb[0].tbs;
		cell_dci.total_ul_tbs_bit += dci_msg.tb[0].tbs;
...
// subframe takes too long to finish.
// In that case, we cannot update the CA header for a long time.
// To prevent that, we need to set a timer
